...
const Layout = () => {
  ...
  // Rutas donde NO debe aparecer el BottomNavigation ni el Header principal
  ...
  // También ocultar en rutas dinámicas de posts y servicios (tienen sus propios headers)
  const isDynamicRoute = location.pathname.startsWith('/posts/') || 
                         (location.pathname !== '/' && 
                          location.pathname !== '/servicios' && 
                          location.pathname !== '/posts' &&
                          !hideNavigationRoutes.includes(location.pathname) &&
                          !location.pathname.startsWith('/implantologia') &&
                          !location.pathname.startsWith('/estetica') &&
                          !location.pathname.startsWith('/urgencias') &&
                          !location.pathname.startsWith('/cupones'))

  const shouldShowNavigation = !hideNavigationRoutes.includes(location.pathname) && !isDynamicRoute

  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header con extensión para Safe Area - Condicional */}
      ...
      <main className={`max-w-7xl mx-auto ${shouldShowNavigation ? 'pb-24' : ''}`}>
        <Outlet />
      </main>

      {/* Bottom Navigation - Condicional */}
